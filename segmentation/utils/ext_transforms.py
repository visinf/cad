...
class ExtRandomHorizontalFlip_w_params(object):
    """Horizontally flip the given PIL Image randomly with a given probability.
    ...
    def __call__(self, img, lbl):
        ...
        Returns:
            PIL Image: Randomly flipped image.
        """
        doFlip = random.random() < self.p
        if doFlip:
            return F.hflip(img), F.hflip(lbl), doFlip
        return img, lbl, doFlip
...
class ExtResizeDevidableByN(object):
    """Resize the input PIL Image s.t. resulting size % N == 0.
    Args:
        N (sequence or int): N
        interpolation (int, optional): Desired interpolation. Default is
            ``PIL.Image.BILINEAR``
    ...
    def __init__(self, N, interpolation=Image.BILINEAR):
        self.N = N
        self.interpolation = interpolation
    ...
    def __call__(self, img, lbl):
        ...
        """
        assert img.size == lbl.size
        target_size = ( int(img.size[1])-(int(img.size[1]) % self.N), int(img.size[0])-(int(img.size[0]) % self.N)) # (H, W)
...
class ExtRandomCrop_w_params(object):
    """Crop the given PIL Image at a random location.
    Args:
        size (sequence or int): Desired output size of the crop. If size is an
            int instead of sequence like (h, w), a square crop (size, size) is
