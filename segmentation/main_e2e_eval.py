...
def validate(opts, model, model_debug, loader, device, metrics, metrics_debug):
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            outputs_debug = model_debug(images)

            preds = outputs.detach().max(dim=1)[1].cpu().numpy()
            preds_debug = outputs_debug.detach().max(dim=1)[1].cpu().numpy()
            targets = labels.cpu().numpy()

            metrics.update(targets, preds)
            metrics_debug.update(targets, preds_debug)

        score = metrics.get_results()
        score_debug = metrics_debug.get_results()
    return score, score_debug, ret_samples
...
def main():
    ...
    metrics_debug = StreamSegMetrics(opts.num_classes)
    ...
    val_score, val_score_debug, ret_samples = validate(
        opts=opts, model=model, model_debug=model_debug, loader=val_loader, device=device, metrics=metrics, metrics_debug=metrics_debug)
    print('For eval mode:', metrics.to_str(val_score))
    print('For train mode:', metrics.to_str(val_score_debug))
    return
